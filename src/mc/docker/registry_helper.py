...
def get_container_registry_credentials(registry_host: str, specified_username: str = None) -> tuple[str, str]:
    """
    ...
    if registry_host in ["docker.io", "index.docker.io", ""]:
        username = specified_username or get_env_var("DOCKERHUB_USERNAME", "")
        password = get_env_var("DOCKERHUB_TOKEN", "")
        return username, password
    elif registry_host in ["ghcr.io"]:
        username = specified_username or get_env_var("GHCR_USERNAME")
        password = get_env_var("GHCR_TOKEN", "")
        return username, password
    elif registry_host.endswith("azurecr.io"):
        username = specified_username or get_env_var("ACR_USERNAME", "")
        password = get_env_var("ACR_PASSWORD", "")
        return username, password
...
def container_registry_login(registry_url: str, username: str = None, password: str = None):
    try:
        ...
        username, password = get_container_registry_credentials(registry_url, username)
