...
def github_get_user_repos(token: str, visibility=None) -> list[dict]:
    """
...
def github_get_org_repos(token: str, org_name: str, visibility=None) -> list[dict]:
    """
    List private GitHub repositories for the authenticated user.
    ...
    page = 1
    while True:
        url = f"https://api.github.com/orgs/{org_name}/repos?per_page=100&page={page}&affiliation=owner"
        if visibility:
            url += f"&visibility={visibility}"
...
def github_list_repos(token: str, visibility=None) -> list[str]:
    ...
    """
    repos = github_get_user_repos(token, visibility)
