...
def ko_playbook_run(project: str, project_path: str, playbook: str, **kwargs):
    ...
    private_data_dir = build_ansible_runner_structure(project_path)
...
def build_ansible_runner_structure(project_path: str):
    ...
    tmp_prefix = os.path.basename(project_path)
    #tmp_dir = tempfile.TemporaryDirectory(prefix=tmp_prefix, delete=False)
    #tmp_private_data_dir = tmp_dir.name
    os.makedirs(TMP_DIR, exist_ok=True)
    tmp_private_data_dir = tempfile.mkdtemp(prefix=tmp_prefix + "-", dir=TMP_DIR)
